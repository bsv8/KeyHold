...
for (const op of manifest) {
  ...
  for (const [sdk, target] of Object.entries(op.sdks)) {
    ...
    const contract = output.sdks[sdk]; const href = `/api/${sdk}/${api.kind}/${api.symbol.replace(/[A-Z]/g, (c) => `-${c.toLowerCase()}`).replace(/^-/, '')}`;
    ...
    const file = path.join(site, sdk === 'typescript' ? 'docs' : 'docs', 'api', sdk, api.kind, `${api.symbol.replace(/[A-Z]/g, (c) => `-${c.toLowerCase()}`).replace(/^-/, '')}.md`); try { await fs.access(file); } catch { fail(`Operation href has no generated route: ${contract.href}`); }
  }
}
