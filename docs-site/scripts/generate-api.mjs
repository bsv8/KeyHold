...
function tsApi(r) {
  ...
  const api = {kind, symbol: r.name, signature: '', summary: refComment(r, `Public ${reflectionKind.toLowerCase()} ${r.name}.`), file: r.sources?.[0]?.fileName ?? '', parameters: [], returns: '', fields: [], methods: []};
  if (kind === 'functions') {
...
async function typedocMetadata() {
  ...
  const apis = reflections.map(tsApi).filter((a, i, all) => all.findIndex((b) => b.symbol === a.symbol && b.kind === a.kind) === i);
  return {sdk: 'typescript', apis};
}
async function goMetadata() {
  const out = path.join(metadataDir, 'go.json');
  await run('go', ['run', path.join(site, 'scripts/go-api-metadata.go'), '-dir', path.join(root, 'go'), '-out', out], {cwd: root});
  const apis = JSON.parse(await fs.readFile(out, 'utf8')).apis;
  const constantSummaries = {Format: 'Format is the KeyHold document format discriminator.', Version: 'Version is the supported KeyHold document version.', KDFAlgorithm: 'KDFAlgorithm identifies the password-based key derivation algorithm.', CipherAlgorithm: 'CipherAlgorithm identifies the authenticated encryption algorithm.'};
  for (const api of apis) { if (!api.summary) api.summary = api.kind === 'constants' ? (constantSummaries[api.symbol] ?? `${api.symbol} is an exported Go constant.`) : `${api.symbol} is a public Go SDK ${api.kind}.`; for (const p of api.parameters ?? []) if (!p.summary) p.summary = `Parameter ${p.name} of ${api.symbol}.`; for (const f of api.fields ?? []) if (!f.summary) f.summary = `Field ${f.name} of ${api.symbol}.`; for (const m of api.methods ?? []) if (!m.summary) m.summary = `Method ${m.name} of ${api.symbol}.`; }
  await fs.writeFile(out, JSON.stringify({apis}, null, 2));
  return {sdk: 'go', apis};
}
const typeLink = (value, sdk, known) => clean(value).split(/(\b[A-Z][A-Za-z0-9_]*\b)/g).map((part, i) => i % 2 && known.has(part) ? `[${part}](/api/${sdk}/types/${slug(part)})` : part).join('').replaceAll('<', '&lt;').replaceAll('>', '&gt;').replaceAll('|', '\\|');
const md = (api, sdk, locale, catalog, known) => {
  const key = `${sdk}.${api.kind}.${api.symbol}`;
...
const generatedOps = operations.map((op) => ({...op, sdks: Object.fromEntries(Object.entries(op.sdks).map(([sdk, target]) => { const api = metadata[sdk].apis.find((a) => a.kind === target.kind && a.symbol === target.symbol); if (!api) throw new Error(`Operation ${op.id} references missing ${sdk} ${target.kind}.${target.symbol}`); return [sdk, {...target, input: api.parameters?.map((p) => `${p.name}: ${p.type}`).join(', ') ?? '', output: api.returns ?? (api.kind === 'types' ? api.fields.map((f) => `${f.name}: ${f.type}`).join(', ') : '') , href: `/api/${sdk}/${api.kind}/${slug(api.symbol)}`}]; }))}));
