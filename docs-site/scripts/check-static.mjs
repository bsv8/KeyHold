...
const generated = JSON.parse(await fs.readFile(path.join(site, 'src/data/operations.generated.json'), 'utf8'));
for (const localePath of ['build/operations.html', 'build/zh-CN/operations.html']) {
  const file = path.join(site, localePath); const html = await fs.readFile(file, 'utf8');
  ...
  for (const operation of generated) { const row = mainRows.find((match) => match[1] === operation.id); if (!row || row[2] !== 'typescript' || row[3] !== operation.sdks.typescript.symbol) throw new Error(`${localePath}: main TypeScript operation row mismatch: ${operation.id}`); }
  const compareRows = html.match(/<tr data-compare-row="[^"]+"/g) ?? []; if (compareRows.length !== 8) throw new Error(`${localePath}: expected exactly 8 compare rows, got ${compareRows.length}`);
}
